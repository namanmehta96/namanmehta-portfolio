...
import { ProjectPreview } from "@/components/effects/ProjectPreview";
  ...
  return (
    <section id="work" className="scroll-mt-24 py-[clamp(4rem,10vw,8rem)]">
      <div className="mx-auto w-full max-w-7xl px-6 md:px-10">
            ...
            <Reveal key={project.slug}>
              <MotionLink
                ...
                onPointerEnter={(event) => {
                  if (event.pointerType === "mouse") setHovered(project.slug);
                }}
              ...
              >
                <div className="grid grid-cols-[1fr_auto] items-start gap-y-4 md:grid-cols-[4.5rem_1fr_auto] md:gap-x-10 md:gap-y-5">
                  <div className="flex items-baseline gap-3 md:col-start-1 md:row-start-1 md:flex-col md:gap-3 md:pt-2">
                    <Counter
                      value={index + 1}
                  ...
                  <span
                    className={`justify-self-end transition-[transform,color] duration-250 group-hover:-translate-y-1 group-hover:translate-x-1 group-focus-visible:-translate-y-1 group-focus-visible:translate-x-1 md:col-start-3 md:row-start-1 md:mt-2 ${
                      project.accent
                        ? "text-accent"
                        : "text-muted group-hover:text-foreground group-focus-visible:text-foreground"
                    }`}
                  ...
                  <p className="col-span-2 max-w-2xl text-muted md:col-span-1 md:col-start-2 md:row-start-2">
                    {project.oneLiner}
                  </p>
                  <div className="col-span-2 md:col-span-1 md:col-start-2 md:row-start-3">
                    <p className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs uppercase tracking-[0.25em] text-muted">
                    ...
                    <div className="mt-5 hidden [@media(pointer:coarse)]:block">
                      <Image
        ...
        {note && (
          <Reveal>
            <div className="grid grid-cols-1 gap-y-3 border-b border-foreground/10 py-[clamp(2rem,4vw,3rem)] md:grid-cols-[4.5rem_1fr] md:gap-x-10 md:gap-y-0">
