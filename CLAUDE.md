...
6. **Progress spine system** — one visual language across About, case studies, and home: a vertical hairline whose amber fill draws downward scrubbed to reading progress (`ProgressSpine`), with circular nodes (`SpineNode`) that sit hollow on the line and fill amber as the fill reaches them, reversing on scroll-back. Case studies pair nodes with the section-number ticks (01 Problem → 04 Learnings); home runs the spine down the work rows aligned with the row numbers. About is the chronology variant of the same language: line at the content edge, always-lit dots, visible on mobile (`TimelineSpine`). Case/home spine + nodes are desktop-only; mobile keeps the number ticks. Functional reading aid — under reduced motion the fill stays empty (never falsely "read"), node fills and ticks are instant. No horizontal top progress bar (tried and removed).
